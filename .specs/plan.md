...
- [ ] **Step 1 — log every run** → `warlock/eval/{metrics,run_logger,cases}.py`, embedding-F, `eval_runs/<date>.jsonl` (gitignored).
  ...
  - [x] Build the logger — `metrics.py` (`coverage`, `routing_precision`, `acceptance_rate`, `output_fidelity` over real memory keys; embedding-F via the `_embedding()` seam using `all-MiniLM-L6-v2`), `run_logger.py` (read-only observer; A/F always, C/R only with a case, `has_case` flag, `iteration` count, append to `eval_runs/<date>.jsonl`, `label=null`). `log_run(m)` wired into `main.py`. Deps added: `numpy`, `sentence-transformers`. Each row is later stamped with run provenance (`git_sha`/`-dirty` via `git status --porcelain`, router `model`, `temperature` from a `run_config` memory key, plus `token_spend`/`timing`) — the "store results like telemetry" upgrade, so re-baselines compare as a time series.
  ...
  - [x] **Gold extended + `assignment_accuracy` made embedding-based + N-sample baseline** (Session 3.5, step 2) — `gold_decomposition` authored for the 5 discriminating cases (bd-01/02/03, md-12, md-13; bd-01 also flipped `verified=True`). `assignment_accuracy` rewritten from keyword-substring to **embedding similarity** (gold deliverable ↔ best expected-domain task, cosine ≥ 0.30, calibrated offline in `calibrate_assignment.py`); the `match` field removed from the gold. A decompose-only N-sample harness (`run_baseline.py`) captured 24×5 → `eval_runs/nsample/2026-06-07.jsonl`. **Result:** embedding @ 0.30 is correct on bd-01/bd-02/bd-03/md-13 and beats keyword on every case; one residual false miss on md-12 (short "CSV export" deliverable diluted in a multi-clause task). **Root cause named:** every matching wart traces to a granularity mismatch (per-deliverable gold vs the orchestrator's one-paragraph-per-domain output); the real fix is deliverable-level orchestrator output (`training_eval_plan.md` §9), deferred to the orchestrator/training rework.

**Track reframed (this session): measure → train.** The objective shifted from *measuring* a run to *training a small LLM* — the **router (orchestrator) first**. The training-oriented plan is `warlock/eval/training_eval_plan.md`. Key consequences: a reward is optimized against, so the bar is higher than for a metric — `acceptance_rate` (inverts against scope) and `output_fidelity` (rewards parroting) are unsafe as rewards; `coverage` has zero variance so recall is untrained; `EvalCase` needs a per-example **`gold_decomposition`** target (a set cannot supervise a sequence model); and the orchestrator currently routes blind (no boundary rules, no `temperature=0` in its prompt). `eval_ml_plan.md`'s D1–D8 classifier still stands — it becomes the **reward model**, distinct from the router task-LLM. The prompt-fix re-baseline control is now **captured** (domain charter + `agent_contracts.md` boundary rules + `temperature=0` in `Orchestrator.decompose`; ~15/24 cases improved on `Routing`, recorded in `eval_runs/2026-06-07.jsonl`), **`assignment_accuracy` is built**, and **`gold_decomposition` is authored for bd-01** (`Assignment` 0.86→1.0). Both next actions from the prior session are now **done** — `gold_decomposition` extended to the 5 discriminating cases and the baseline **N-sampled** (`eval_runs/nsample/2026-06-07.jsonl`); `assignment_accuracy` is now embedding-based. Remaining 3.5 items: scope-aware `acceptance_rate` (Fix A) + fence `output_fidelity` out of any reward, and under-routing / hard-negative cases so `coverage` gains variance. The standing structural fix is **deliverable-level orchestrator output** (exact assignment scoring + SFT-shaped output), to land with the orchestrator/training rework.
